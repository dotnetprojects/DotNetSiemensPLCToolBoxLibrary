...
namespace DotNetSiemensPLCToolBoxLibrary.DBF {
    ...
    public class NodeFloat:IComparable {
        ...
        public int CompareTo(object obj){
            ...
            if (obj.GetType() == typeof(NodeFloat)){
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    if (bValue[i] < 0){  // reverse all logic for bytes whose first nibble is 8 or 9
                    
                        if ((nf2.bValue[i] < 0) && (nf2.bValue[i] < bValue[i]))  // though it's smaller it's bigger
                            return -1;
                        return 1;
                    }
                    if (nf2.bValue[i] < 0){  // reverse all logic for bytes whose first nibble is 8 or 9
        ...
        public string GetString {
            get{
                return this.dSaveValue.ToString();
            }
        }
